...
pub trait FieldTranscript<F: Field>: Pattern {
    ...
    fn message_scalars(&mut self, label: impl AsRef<str>, input: &[F]) -> &mut Self {
        self.begin_message::<F>(&label, Length::Fixed(input.len()));
    ...
    fn challenge_scalars(&mut self, label: impl AsRef<str>, output: &mut [F]) -> &mut Self {
        self.begin_challenge::<F>(&label, Length::Fixed(output.len()));
    ...
    fn message_public_scalars(&mut self, label: impl AsRef<str>, input: &[F]) -> &mut Self {
        self.begin_public::<F>(&label, Length::Fixed(input.len()));
...
pub trait VerifierFieldTranscript<F: Field>: FieldTranscript<F> {
    ...
    fn read_message_scalars(
        ...
        self.begin_message::<F>(&label, Length::Fixed(output.len()));
...
pub trait GroupTranscript<G: CurveGroup>: Pattern {
    ...
    fn message_points(&mut self, label: impl AsRef<str>, input: &[G]) -> &mut Self {
        self.begin_message::<G>(&label, Length::Fixed(input.len()));
    ...
    fn message_public_points(&mut self, label: impl AsRef<str>, input: &[G]) -> &mut Self {
        self.begin_public::<G>(&label, Length::Fixed(input.len()));
...
pub trait VerifierGroupTranscript<G: CurveGroup>: GroupTranscript<G> {
    ...
    fn read_message_points(
        ...
        self.begin_message::<G>(&label, Length::Fixed(output.len()));
